...
{
  withProgress(message = 'Protein Domains analysis ... ', value = 0, {
  incProgress(1/2, detail = "Customed SEA ...")    
...
proteinDomain <- function(input, output, session, org, organismsDbKegg, pvalues, logF, entrezID, minGS, maxGS, nPerm, pvAdjustMethod)
{
  ...
  incProgress(1/2, detail = "Biomart query ...")    
  ...
  ensembl = useEnsembl(biomart="ensembl", dataset = dataset$dataset, host="uswest.ensembl.org")
  ...
  withProgress(message = 'Motif GSEA ... ', value = 0, {
    ...
    # Download table
    output$dl.Motif <- downloadHandler(
      filename = "proteinMotifsResults_GSEA.csv",
      content = function(filename) {
        write.csv(motifs.GSEA, filename, row.names = T)
      })
